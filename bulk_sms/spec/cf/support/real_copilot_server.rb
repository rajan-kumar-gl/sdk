...
class RealCopilotServer
  ...
  def initialize
    ...
    config = {
      ...
      'ServerCertPath' => fixture('clients-server.crt'),
      'ServerKeyPath' => fixture('clients-server.key'),
      'VIPCIDR' => "127.128.0.0/9",
      'BBS' => { 'Disable' => true },
      'LogLevel' => 'fatal',
      'PolicyServerDisabled' => true,
      'PolicyServerAddress' => 'https://policy-server.service.cf.internal:4003',
      'PolicyServerClientCertPath' => fixture('clients-server.crt'),
      'PolicyServerClientKeyPath' => fixture('clients-server.key'),
      'PolicyServerCAPath' => fixture('fakeCA.crt')
    }

    config_file = Tempfile.new('clients-config')
    config_file.write(config.to_json)
    config_file.close

    @copilot_server = fork do
      exec "clients-server -config #{config_file.path}"
    end
